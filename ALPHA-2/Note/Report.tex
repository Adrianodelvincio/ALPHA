...
\begin{abstract}
\centering
A fit procedure to separate the $\overline{H}$ annihilations on ALPHA trap walls from residual gas annihilations has been implemented and tested. Moreover, a Monte Carlo simulation concerning the hyperfine splitting measurement was designed, with the intention of studying the bias and statistical uncertainty of various algorithms to find frequency onset of the energy-level transitions $c \rightarrow b$ and $d \rightarrow a$ of anti-hydrogen.
\end{abstract}
\end{@twocolumnfalse}
%\hspace{10pt}
]
