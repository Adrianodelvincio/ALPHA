{
  ...
  int irun= 68481; // 68498; // 68489 ; // run 68489
  ...
  TA2Plot plotMW8(true);  plotMW8.AddDumpGates(irun, {"MW Frq 8 Dump"}, {-1});   plotMW8.LoadData();
  plotMW8.ExportCSV(TString::Format("r%d_uw_exp_freq8",irun).Data());
  
  TA2Plot plotMW9(true);  plotMW9.AddDumpGates(irun, {"MW Frq 9 Dump"}, {-1});   plotMW9.LoadData();
  plotMW9.ExportCSV(TString::Format("r%d_uw_exp_freq9",irun).Data());
  
  TA2Plot plotMW10(true);  plotMW10.AddDumpGates(irun, {"MW Frq 10 Dump"}, {-1});   plotMW10.LoadData();
  plotMW10.ExportCSV(TString::Format("r%d_uw_exp_freq10",irun).Data());
  
  TA2Plot plotMWOR(true);  plotMWOR.AddDumpGates(irun, {"MW OffResonance Dump"}, {-1});   plotMWOR.LoadData();
  plotMWOR.ExportCSV(TString::Format("r%d_uw_exp_offres",irun).Data());
  ...
  TA2Plot plotcMW8(true);  plotcMW8.AddDumpGates(irun, {"c MW Frq 8 Dump"}, {-1});   plotcMW8.LoadData();
  plotcMW8.ExportCSV(TString::Format("r%d_c_uw_exp_freq8",irun).Data());
  
  TA2Plot plotcMW9(true);  plotcMW9.AddDumpGates(irun, {"c MW Frq 9 Dump"}, {-1});   plotcMW9.LoadData();
  plotcMW9.ExportCSV(TString::Format("r%d_c_uw_exp_freq9",irun).Data());
  
  TA2Plot plotcMW10(true);  plotcMW10.AddDumpGates(irun, {"c MW Frq 10 Dump"}, {-1});   plotcMW10.LoadData();
  plotcMW10.ExportCSV(TString::Format("r%d_c_uw_exp_freq10",irun).Data());
   
  TA2Plot plotcMWOR(true);  plotcMWOR.AddDumpGates(irun, {"c MW OffResonance Dump"}, {-1});   plotcMWOR.LoadData();
  plotcMWOR.ExportCSV(TString::Format("r%d_c_uw_exp_offres",irun).Data());
  ...
  // // The one below is followed by more stacking.
  ...
  TA2Plot plotUWpre1(true);  plotUWpre1.AddDumpGates(irun, {"Microwave Dump"}, {1});   plotUWpre1.LoadData();
  plotUWpre1.ExportCSV(TString::Format("r%d_prelaser_uw_0",irun).Data()); 

  TA2Plot plotUW0(true);  plotUW0.AddDumpGates(irun, {"Microwave Dump"}, {2});   plotUW0.LoadData();
  plotUW0.ExportCSV(TString::Format("r%d_postlaser_uw_0",irun).Data());
  
  TA2Plot plotUW1(true);  plotUW1.AddDumpGates(irun, {"Microwave Dump"}, {3});   plotUW1.LoadData();
  ...
  TA2Plot plot11(true);   plot11.AddTimeGate(irun, spillsUW.at(1).GetStopTime(),tcyclestart);   plot11.LoadData();  
  ...
  TA2Plot plot6(true);   plot6.AddTimeGate(irun, tcyclestop, spillsUW.at(2).GetStartTime());   plot6.LoadData();
  plot6.ExportCSV(TString::Format("r%d_postlaser_preuw",irun).Data()); 

  TA2Plot plot7(true);   plot7.AddTimeGate(irun, spillsUW.at(2).GetStopTime(), spillsUW.at(3).GetStartTime());   plot7.LoadData();
  plot7.ExportCSV(TString::Format("r%d_inter_uw",irun).Data()); 

  TA2Plot plot5(true);   plot5.AddTimeGate(irun, spillsUW.at(3).GetStopTime(), spillsFRD.at(0).GetStartTime());   plot5.LoadData();
